...
// Lógica da tela de evolução e gráficos
// ============================================
...
function renderizarStats() {
  ...
  const exerciciosUnicos = new Set(
    h.flatMap(t => t.exercicios.map(e => e.nome))
  ).size

  document.getElementById("stats-grid").innerHTML = `
    <div class="stat">
      <div class="stat-numero">${totalTreinos}</div>
      <div class="stat-label">treinos</div>
    </div>
    <div class="stat">
      <div class="stat-numero">${Math.round(totalPeso / 1000)}t</div>
      <div class="stat-label">peso total</div>
    </div>
    <div class="stat">
      <div class="stat-numero">${Math.round(totalMin / 60)}h</div>
      <div class="stat-label">na academia</div>
    </div>
    <div class="stat">
      <div class="stat-numero">${exerciciosUnicos}</div>
...
function renderizarFrequencia() {
  ...
  historicoGlobal.forEach(t => {
    const iso = t.dataISO || ""
    if (!iso) return
    const chave = iso.slice(0, 7) // "2024-03"
  ...
  chartFrequencia = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          ticks: { stepSize: 1, font: { family: "Space Grotesk", size: 11 } },
          grid: { color: "#f0ebe0" }
        },
        x: {
...
function renderizarGrupamento() {
  ...
  const bgColors = labels.map(l => (cores[l] || "#ccc") + "55")
  const bdColors = labels.map(l => cores[l] || "#ccc")
...
function preencherSelectExercicios() {
  ...
  const ordenados = Object.entries(exercicios)
    .sort((a, b) => b[1] - a[1])
    .map(e => e[0])
  ...
  // Seleciona o mais frequente automaticamente
...
function atualizarGraficoCarga() {
  ...
  historicoGlobal
    ...
    .forEach(t => {
      ...
      pontos.push({
        x: t.dataISO,
        y: ex.cargaMax || ex.carga || 0,
        label: t.data || t.dataISO
      })
    })
  ...
  chartCarga = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        ...
        x: {
          ticks: { font: { family: "Space Grotesk", size: 10 }, maxRotation: 45 },
          grid: { display: false }
        }
      }
    }
...
function renderizarRecordes() {
  ...
  const ordenados = Object.entries(recordes)
    .sort((a, b) => b[1].carga - a[1].carga)
    .slice(0, 10)
...
function confirmarLimpar() {
  if (confirm("Tem certeza? Isso vai apagar todos os treinos salvos no navegador. Exporta antes se quiser guardar!")) {
